...
% -------------------------------------------------------------------------------------------------
    ...
    %% network struct
    p.init.scale = 1;
    p.init.weightInitMethod = 'xavierimproved';
    p.init.initBias = 0.1;
    p.conf = struct(); % Options depend on type of net.
...
% -------------------------------------------------------------------------------------------------
    ...
    % motion branch computes features for search region x_t,x_t-1,x_t-2 and
    % cross-correlates with off feature
    % create motion network
    net_m = make_msiamfc(p);
    ...
    x_crops_m = gpuArray(zeros(p.instanceSize,p.instanceSize,3,2,'single'));
    z_crops_m = gpuArray(zeros(p.exemplarSize,p.exemplarSize,3,2,'single'));
    z_imgs{1} = im;z_imgs{2} = im;
    x_imgs{1} = im;
    pos_m(1,:)= targetPosition;
    sz_m(1,:) = targetSize;
    pos_m(2,:)= targetPosition;
    sz_m(2,:) = targetSize;
    % start tracking
    tic;
    for i = startFrame:nImgs
        if i>startFrame
            ...
            %% get motion response map
            if i>3 && p.msiam
                x_imgs{2}=im;
                mot_response = gen_mot_response(net_m,x_imgs,z_imgs,targetPosition_m,targetPosition,targetSize_m,s_x_m,s_x,s_z_m,avgChans,stats,p);
            else
                mot_response = gpuArray(single(ones(p.scoreSize,p.scoreSize)));
            end
            
            %% locate target with siamfc
            % evaluate the offline-trained network for exemplar x features
            [newTargetPosition, newScale] = tracker_eval(net_x, round(s_x), scoreId, z_features, x_crops, targetPosition, window, mot_response,p);
            scale_index = newScale;
        ...
        if i>startFrame && p.msiam
            ...
            targetSize_m(1) = max(pos_m(:,1)+sz_m(:,1)./2)-min(pos_m(:,1)-sz_m(:,1)./2); 
            targetSize_m(2) = max(pos_m(:,2)+sz_m(:,2)./2)-min(pos_m(:,2)-sz_m(:,2)./2); 
            targetPosition_m(1) = min(pos_m(:,1)-sz_m(:,1)./2)+targetSize_m(1)./2;
            targetPosition_m(2) = min(pos_m(:,2)-sz_m(:,2)./2)+targetSize_m(2)./2;
            wc_z_m = targetSize_m(2) + p.contextAmount*sum(targetSize_m);
            hc_z_m = targetSize_m(1) + p.contextAmount*sum(targetSize_m);
            s_z_m = sqrt(wc_z_m*hc_z_m);
            scale_z_m = p.exemplarSize / s_z_m;
            d_search = (p.instanceSize - p.exemplarSize)/2;
            pad_m = d_search/scale_z_m;
            s_x_m = s_z_m + 2*pad_m;
        end
